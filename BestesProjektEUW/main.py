...
class USBInterface:
    def __init__(self, port, baudrate):
        ...
        self.temp_label = tkinter.Label(self.tkTop, text="Aktuelle Temperatur:")
        self.temp_sensor_data_label = tkinter.Label(self.tkTop, text="0", )
        self.temp_status = tkinter.Label(self.tkTop, text="N/A", )
    ...
    def create_gui(self):
        ...
        self.temp_label.grid(column=1, row=4, padx=1, pady=15, sticky="W")
        self.temp_sensor_data_label.grid(column=2, row=4, padx=1, pady=15, sticky="W")
        self.temp_status.grid(column=3, row=4, padx=1, pady=15, sticky="W")

        self.arduino_info_label.grid(column=1, row=5, padx=1, pady=15, sticky="W")
        ...
        temp_button = tkinter.Button(
            self.tkTop,
            text="Request_temperature",
            command=self.request_temperature_state,
            height=1,
            fg="black",
            width=20,
            bd=1
        )
        temp_button.grid(column=0, row=4, ipadx=10, padx=10, pady=15, sticky="W")
    ...
    def request_temperature_state(self):
        self.temp_sensor_data_label.config(text=self.actual_temperature)
        if self.actual_temperature <= 0:
            self.temp_status.config(text="Something went off with the temperature-sensor, \nif pressing "
                                                 "'request_temperature_state' again doesn't help, please check!")
        else:
            self.temp_status.config(text="Temperature-sensor is working properly.")
